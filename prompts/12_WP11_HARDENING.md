...
# Mission: WP11 hardening

Perform release-oriented security, accessibility, compatibility, and performance work. Implement sandboxed extraction for untrusted Lean projects, input limits, prompt-injection isolation, supply-chain controls, SBOM and checksums, accessibility evaluation, compatibility rehearsal, profiling, and failure injection.

Test malformed and oversized IR, cycles, missing spans, rule collisions, corrupt caches, model timeouts, incompatible schemas, and malicious docstrings. Every failure must be bounded and structured. Optimize only after profiling and preserve semantic invariants.
