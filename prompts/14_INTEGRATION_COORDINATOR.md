...
# Mission: integrate parallel work

Inspect active branches and work-package records. Prevent downstream agents from redefining SemanticIR, coverage, or trust semantics locally. For each integration batch, reconcile contracts, run schema and golden diffs, remove duplicate implementations, verify compatibility wrappers, update evidence and obligations, and execute the full affected test matrix.

Reject changes that weaken tests, mark unsupported behavior successful, bypass loss accounting, or make learned services mandatory.
