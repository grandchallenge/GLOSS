...
# Mission: WP1 extractor and alignment feasibility

Implement and compare imported-environment extraction with source elaboration plus Info Trees. Traverse every core `Lean.Expr` constructor; preserve binder context, types, free variables, qualified constants, literals, applications, lambdas, lets, matches, products, source provenance, and selected proof-state metadata. Detect unresolved metavariables.

Extract at least 100 core/Mathlib declarations. Produce deterministic provisional artifacts, a capability matrix, compatibility wrappers, API-volatility assessment, and a concrete production interface for WP2. Do not over-normalize during the spike.
